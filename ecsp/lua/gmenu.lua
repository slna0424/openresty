...
local function send_response(request, ctx, response)
	local send_msg,menulist  = {}, {};
	local chanl_img, chanl_logo = "", "";
	send_msg.term_no = request.post_fields.term_no or "";        --终端编号
	local index = (response.msg and #response.msg) or 0;
	for i = 1, index do
		chanl_img = string.format("%s/%s",path.pic_path,response.msg.seller_name);
		chanl_logo = string.format("%s/%s",path.pic_path,response.msg.seller_logo);
		local tmp = {chanl_no = response.msg.shop_id, chanl_img = chanl_img, chanl_logo = chanl_logo};
		table.insert(menulist, tmp);	
	end
	send_msg.l_chinfo = menulist;
